...
//---------------------------------------------------------------------------------------
class XComGameState_Unit_AlienCustomization extends XComGameState_BaseObject dependson(X2EventListener_AlienCustomization);
...
simulated function GenerateCustomization(UnitVariation UnitVar, XComGameState_Unit UnitState, XComGameState NewGameState)
{
	//save off body part and material customization so it can be re-applied after save/load cycle
	m_kAppearance = UnitVar.BodyPartContent;
	fScale = UnitVar.Scale;
	bAutomatic = UnitVar.Automatic;
	BodyAppearance = UnitVar.BodyAppearance;
	GenericBodyPartArchetypes = UnitVar.GenericBodyPartArchetypes;
	PrimaryWeaponAppearance = UnitVar.PrimaryWeaponAppearance;
	SecondaryWeaponAppearance = UnitVar.SecondaryWeaponAppearance;
}
...
simulated function bool ApplyCustomization(optional XComUnitPawn PawnToUpdate, optional bool bUpdateItems = true)
{
	...
	{
		bSuccess = false;
		`REDSCREEN("AlienCustomization: Failed to apply Body Material Customization.");
	}
...
function bool ApplyItemMaterialCustomization(XComGameState_Item ItemState, ObjectAppearance Appearance, optional XComWeapon WeaponPawn)
{
...
simulated function UpdateMaterials(MeshComponent MeshComp, ObjectAppearance Appearance)
{
	local int i;
...
simulated function UpdateIndividualMaterial(MeshComponent MeshComp, MaterialInstanceConstant MIC, ObjectAppearance Appearance)
{
	local MaterialInterface ParentMat;
	local MaterialInstanceConstant ParentMIC;
	local name ParentName;
	local ColorParameter ColorParameter;
	local ScalarParameter ScalarParameter;
	...
	if (MIC != none)
	{
		...
		switch (ParentName)
		{
			...
			case 'M_Master_PwrdWep_TC':
				...
				{
					`LOG("Alien Customization: Setting Vector Parameter.");
...
{	
	...
	{		
		...
		if (PartTemplate != none) //If the part couldn't be found, then just ignore it. This could happen when loading a save that had DLC installed that we don't have
		{
			...
			{
				`LOG("AlienVariations: Loading content " $ PartTemplate.ArchetypeName);
