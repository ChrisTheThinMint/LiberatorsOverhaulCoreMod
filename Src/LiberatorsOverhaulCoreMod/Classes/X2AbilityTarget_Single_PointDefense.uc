...
class X2AbilityTarget_Single_PointDefense extends X2AbilityTarget_Single config (GameData_SoldierSkills);

var config int POINT_DEFENSE_RANGE;
var config bool POINT_DEFENSE_PROC_ON_OWN_TURN;
...
simulated function name GetPrimaryTargetOptions(const XComGameState_Ability Ability, out array<AvailableTarget> Targets)
{
	...
	for (i = Targets.Length - 1; i >= 0; --i)
	{
		...
		if (CCS_ShooterUnit.TileDistanceBetween(TargetUnit) > default.POINT_DEFENSE_RANGE)
		{
			Targets.Remove(i,1);
		}								
	}	
...
simulated function bool ValidatePrimaryTargetOption(const XComGameState_Ability Ability, XComGameState_Unit SourceUnit, XComGameState_BaseObject TargetObject)
{
	...
	if (b_valid)
	{
		...
		if (SourceUnit.TileDistanceBetween(TargetUnit) > default.POINT_DEFENSE_RANGE)
		{
			//`LOG ("NO SHOT! CCS Target TDB:" @ string(SourceUnit.TileDistanceBetween(TargetUnit)));
			return false;
		}
		if(!default.POINT_DEFENSE_PROC_ON_OWN_TURN && X2TacticalGameRuleset(XComGameInfo(class'Engine'.static.GetCurrentWorldInfo().Game).GameRuleset).GetCachedUnitActionPlayerRef().ObjectID == SourceUnit.ControllingPlayer.ObjectID)
		{
			return false;
		}
	}
